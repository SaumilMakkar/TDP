(function () {
  ...
  function initRowNavigation() {
    ...
    rows.forEach(function (row) {
      ...
      row.addEventListener('click', function (event) {
        ...
        if (rx) {
          window.location.href = '/review/' + encodeURIComponent(rx);
        }
      });
    });
  }
  ...
  function rebuildDueSummaryFromRows() {
    ...
    var todayCountNode = document.querySelector('.due-summary-row .due-summary-count');
    var weekCountNodes = document.querySelectorAll('.due-summary-row .due-summary-count');
    if (todayCountNode) {
      todayCountNode.textContent = String(todayCount);
    }
    if (weekCountNodes.length > 1) {
      weekCountNodes[1].textContent = String(weekCount);
    }
  ...
  function renderDueDay(day, grouped) {
    ...
    var todayObj = new Date(todayParts[2], parseInt(todayParts[0]) - 1, todayParts[1]);
  ...
  function initDayTabs() {
    ...
    var todayDate = window.__TODAY_DATE__ || '';
    var dayDates = window.__DAY_DATES__ || {};
    var prescriptions = window.__PRESCRIPTIONS__ || [];
    var providerData = window.__PROVIDER_DATA__ || {};

    dayTabs.forEach(function (tab) {
      var day = tab.getAttribute('data-day');
      var dayDate = dayDates[day] || '';
      var itemsForDay = providerData[day] || [];
      
      // Parse dates for comparison
      var todayParts = todayDate.split('/');
      var todayObj = new Date(todayParts[2], parseInt(todayParts[0]) - 1, todayParts[1]);
      
      var dayParts = dayDate.split('/');
      var dayObj = new Date(dayParts[2], parseInt(dayParts[0]) - 1, dayParts[1]);
      
      // Determine day status
      var isToday = todayDate === dayDate;
      var allCompleted = true;
      
      if (itemsForDay.length > 0) {
        // Check items for this day
        for (var i = 0; i < itemsForDay.length; i++) {
          var item = itemsForDay[i];
          // If item is not completed, it's still pending
      ...
      if (allCompleted && itemsForDay.length > 0) {
